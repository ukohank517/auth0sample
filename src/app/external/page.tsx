    ...
    <>
      <div className="mb-5" data-testid="external">
        ...
        <div data-testid="external-text">
          <p className="lead">下記ボタンをクリックして、外部APIへ通信する</p>
          <p>
            このページは、外部APIへのリクエストを行うために、アクセストークンを使用します。アクセストークンは、リクエストの <code>Authorization</code> ヘッダーの一部として送信され、API はそれを API の audience 値を使用して検証します。audience は、呼び出したい API の識別子です（詳細は <a href="https://auth0.com/docs/get-started/dashboard/tenant-settings#api-authorization-settings">API Authorization Settings</a> を参照）。
          </p>
          <p>
